...
export class OrcaProvider implements MarketProvider {
  public name = 'orca';
    ...
    return this.rateLimiter.throttle(async () => {
      return exponentialBackoff(async () => {
        try {
          ...
          const orcaPools = pools.filter((p: any) => p.relationships?.dex?.data?.id === 'solana_orca');
          const tokens = this.transformPools(orcaPools, included);
          return tokens.length > 0 ? tokens[0] : null;
        } catch (err: any) {
          console.error(`[Orca] fetchToken failed for ${address}:`, err.message);
          return null;
        }
      });
    });
  ...
  async fetchMarkets(): Promise<TokenData[]> {
    return this.rateLimiter.throttle(async () => {
      return exponentialBackoff(async () => {
        try {
          const res = await httpClient.get(`${this.baseURL}/networks/solana/dexes/orca/pools`, {
            params: { include: 'base_token', page: 1 }
          });
          const pools = res.data.data || [];
          const included = res.data.included || [];
          ...
          console.error('[Orca] fetchMarkets failed:', err.message);
          return [];
        }
      });
    });
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const response = await httpClient.get(`${this.baseURL}/networks/solana/dexes/orca`);
      return response.status === 200;
    } catch (err: any) {
      console.error('[Orca] healthCheck failed:', err.message);
      return false;
    }
  }
...
export default new OrcaProvider();
