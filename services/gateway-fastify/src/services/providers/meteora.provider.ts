...
export class MeteoraProvider implements MarketProvider {
    ...
    return this.rateLimiter.throttle(async () => {
      return exponentialBackoff(async () => {
        try {
          ...
          const meteoraPools = pools.filter((p: any) => p.relationships?.dex?.data?.id === 'solana_meteora');
          const tokens = this.transformPools(meteoraPools, included);
          return tokens.length > 0 ? tokens[0] : null;
        } catch (err: any) {
          console.error(`[Meteora] fetchToken failed for ${address}:`, err.message);
          return null;
        }
      });
    });
  ...
  async fetchMarkets(): Promise<TokenData[]> {
    return this.rateLimiter.throttle(async () => {
      return exponentialBackoff(async () => {
        try {
          const res = await httpClient.get(`${this.baseURL}/networks/solana/dexes/meteora/pools`, {
            params: { include: 'base_token', page: 1 }
          });
          const pools = res.data.data || [];
          const included = res.data.included || [];
          ...
          console.error('[Meteora] fetchMarkets failed:', err.message);
          return [];
        }
      });
    });
...
export default new MeteoraProvider();
